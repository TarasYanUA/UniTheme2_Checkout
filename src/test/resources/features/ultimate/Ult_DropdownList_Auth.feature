...
Feature: Чекаут со способом доставки и способом оплаты в виде Выпадающего списка + Авторизованный пользователь
  ...
  Scenario: Чекаут со способом доставки и способом оплаты в виде Выпадающего списка + Авторизованный пользователь
    ...
    And Выбираем способ доставки "Самовывоз" из выпадающего списка и выбираем пункт выдачи (скриншот "@10")
    And Выбираем способ оплаты "Обсудить по телефону" из выпадающего списка (скриншот "@10")
    And Ставим соглашения (проверяем на уникальность ID)
    Then Завершаем оформление заказа и проверяем, что мы на странице "Ваш заказ принят" (скриншот "@10")