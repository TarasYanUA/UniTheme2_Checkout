@30
Feature: Чекаут со способом доставки и способом оплаты в виде Обычного списка + Авторизованный пользователь
  ...
  Scenario: Чекаут со способом доставки и способом оплаты в виде Обычного списка + Авторизованный пользователь
    ...
    Then Завершаем оформление заказа и проверяем, что мы на странице "Ваш заказ принят" (скриншот "@30")