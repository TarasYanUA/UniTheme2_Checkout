@MV120
Feature:(MV) Чекаут со способом доставки и способом оплаты в виде выпадающего списка + Неавторизованный пользователь
  ...
  Scenario:(MV) Чекаут со способом доставки и способом оплаты в виде выпадающего списка + Неавторизованный пользователь
    ...
    And Выбираем способ доставки "Обсудить с менеджером" для первого продавца из "выпадающего" списка (скриншот "@110")
    And Выбираем способ "доставки" "Самовывоз" из выпадающего списка (скриншот "@120")
    And Выбираем способ "оплаты" "Обсудить по телефону" из выпадающего списка (скриншот "@120")
    And Ставим соглашения (проверяем на уникальность ID)
    Then Завершаем оформление заказа и проверяем, что заказ оформлен успешно (скриншот "@120")